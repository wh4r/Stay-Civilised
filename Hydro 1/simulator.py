...
class HydroSimulator(QMainWindow):
    def __init__(self):
        ...
        controls_layout.addWidget(self.decrease_gate)
        controls_layout.addWidget(self.stop_gate)
        controls_layout.addWidget(self.increase_gate)
    ...
    def update_simulation(self):
        ...
        if not self.is_emergency and self.gate_direction != 0:
            self.gate_opening = max(0, min(100, self.gate_opening + (self.gate_direction * 0.2)))

        if not self.sync:
            # physics based on gate opening
            target_rpm = (self.gate_opening * 12.0) if not self.is_emergency else 0.0
            # Smoothly interpolate current RPM to target (simulating inertia)
            self.current_rpm += (target_rpm - self.current_rpm) * 0.005
        else:
            # slow rpm change
            target_rpm = 500
